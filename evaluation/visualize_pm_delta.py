...
class PMHistogramState(object):
    def __init__(self, di, delta, x_parent, samples, nbins):
        self.di = di
        self.delta = delta
    ...
    def _sample(self):
        operator = pm_delta(0.0, 1.0, self.delta, self.di)
    ...
    def update_di(self, di):
        ...
        if di == self.di:
            return self
        new_state = PMHistogramState(di, self.delta, self.x_parent, self.samples, self.nbins)
        return new_state

    def update_delta(self, delta):
        """
        return a new state with the new delta
        """
        if delta == self.delta:
            return self
        new_state = PMHistogramState(self.di, delta, self.x_parent, self.samples, self.nbins)
        return new_state
    ...
    def update_x_parent(self, x_parent):
        ...
        if x_parent == self.x_parent:
            return self
        new_state = PMHistogramState(self.di, self.delta, x_parent, self.samples, self.nbins)
        return new_state
    ...
    def update_samples(self, samples):
        ...
        if samples == self.samples:
            return self
        new_state = PMHistogramState(self.di, self.delta, self.x_parent, samples, self.nbins)
        return new_state
    ...
    def update_nbins(self, nbins):
        ...
        if nbins == self.nbins:
            return self
        new_state = PMHistogramState(self.di, self.delta, self.x_parent, self.samples, nbins)
...
class OperatorInspectionFrame(QFrame):
    def __init__(self, *args, **kwargs):
        ...
        # prepare state
        initial_state = PMHistogramState(15, 0.1, 0.5, 10000, 16)
        ...
        self.delta_label = QLabel(control_panel)
        cp_layout.addWidget(self.delta_label)
        self.delta_slider = QSlider(Qt.Horizontal, control_panel)
        cp_layout.addWidget(self.delta_slider)
        self.delta_slider.setMinimum(1)
        self.delta_slider.setMaximum(50)
        self.delta_label.setText("delta {}".format(initial_state.delta))
        self.delta_slider.setValue(initial_state.delta)
        self.delta_slider.valueChanged.connect(self._delta_changed)
    ...
    def _delta_to_slider(self, delta):
        slider = int(delta * 100)
        return slider

    def _slider_to_delta(self, slider):
        delta = 0.01 * slider
        return delta

    def _delta_changed(self, slider_position):
        delta = self._slider_to_delta(slider_position)
        new_state = self.states[self.state_index].update_delta(delta)
        self.delta_label.setText("{} delta".format(delta))
        self.do(new_state)
