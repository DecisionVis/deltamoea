...
class SBXHistogramState(object):
    ...
    def _sample(self):
        def operator(x_parent1, x_parent2):
            x1 = int(numpy.floor(x_parent1 * self.nbins))
            x2 = int(numpy.floor(x_parent2 * self.nbins))
            integer_output = sbx_index(x1, x2, self.nbins, random)
            return integer_output * 1.0 / self.nbins
    ...
    def update_x_parent1(self, x_parent1):
        ...
        """
        updated_value = int(numpy.floor(x_parent1 * self.nbins))
        if updated_value == int(numpy.floor(self.x_parent1 * self.nbins)):
            return self
        if updated_value == int(numpy.floor(self.x_parent2 * self.nbins)):
            return self
        new_state = SBXHistogramState(
    ...
    def update_x_parent2(self, x_parent2):
        ...
        return a new state with the new parent X
        """
        updated_value = int(numpy.floor(x_parent2 * self.nbins))
        if updated_value == int(numpy.floor(self.x_parent1 * self.nbins)):
            return self
        if updated_value == int(numpy.floor(self.x_parent2 * self.nbins)):
            return self
